...
const retroConfig: Record<string, { border: string; label: string; icon: string }> = {
  Improvements: { border: "border-t-amber-400 dark:border-t-amber-500", label: "text-amber-700 dark:text-amber-400", icon: "⚙" },
  Regrets:      { border: "border-t-rose-400 dark:border-t-rose-500",   label: "text-rose-700 dark:text-rose-400",   icon: "△" },
  "Next Steps": { border: "border-t-blue-400 dark:border-t-blue-500",   label: "text-blue-700 dark:text-blue-400",   icon: "→" },
};
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16 space-y-20">
      ...
      <section>
        ...
        <div className="space-y-4 text-sm leading-7 text-neutral-700 dark:text-neutral-300">
          ...
          <div className="p-4 rounded-lg bg-neutral-50 dark:bg-neutral-900/60 border border-neutral-200 dark:border-neutral-800 border-l-4 border-l-emerald-400 dark:border-l-emerald-600">
            <p className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wide mb-2">Design Rationale</p>
          ...
          {project.problemSolving.map((ps, i) => (
            <div key={i} className="relative pl-6 border-l-2 border-emerald-200 dark:border-emerald-800">
              ...
              <dl className="space-y-3 text-sm">
                <div>
                  <dt className="font-semibold text-neutral-900 dark:text-neutral-100 mb-1">Issue</dt>
                  <dd className="text-neutral-700 dark:text-neutral-300 leading-relaxed">{ps.issue}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-neutral-900 dark:text-neutral-100 mb-1">Analysis</dt>
                  <dd className="text-neutral-700 dark:text-neutral-300 leading-relaxed">{ps.analysis}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-neutral-900 dark:text-neutral-100 mb-1">Solution</dt>
                  <dd className="text-neutral-700 dark:text-neutral-300 leading-relaxed">{ps.solution}</dd>
                </div>
                <div className="pt-1">
                  <dt className="font-semibold text-neutral-900 dark:text-neutral-100 mb-1">Result</dt>
      ...
      <section className="pb-8">
        <SectionTitle>Retrospective</SectionTitle>
        <div className="grid sm:grid-cols-3 gap-4">
          <RetroCard label="Improvements" value={project.retrospective.improvements} />
          <RetroCard label="Regrets"      value={project.retrospective.regrets} />
          <RetroCard label="Next Steps"   value={project.retrospective.futureWork} />
        </div>
      </section>
