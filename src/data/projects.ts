...
export const projects: Project[] = [
  {
    ...
    overview: {
      ...
      role: "프론트엔드(Next.js), 백엔드(Spring Boot), 인프라(Terraform), CI/CD(GitHub Actions)까지 전 과정을 단독으로 설계·구현·운영하고 있습니다. 기능 구현보다 '혼자서도 안정적으로 운영할 수 있는 구조'를 만드는 데 더 많은 시간을 쏟았습니다.",
    },
    architecture: {
    ...
    techStack: [
      ...
      },
      {
        ...
        role: "에러 자동 분석 및 알림",
        reason:
          "에러 로그를 그대로 메일로 받으면 '이게 긴급한 건지, 무시해도 되는 건지' 판단이 매번 필요했습니다. Haiku 모델로 원인 분석과 심각도 판단을 자동화했고, 비용 통제를 위해 컨텍스트 50줄 제한, 10분 중복 쿨다운, async 스레드풀 3개 상한을 걸었습니다. AI를 쓰되 비용이 예측 불가능해지지 않도록 한 설계입니다.",
      },
      {
        name: "Terraform",
        role: "AWS 인프라 전체 IaC",
      ...
      },
      {
        ...
        role: "빌드·테스트·배포 자동화",
        reason:
          "CI는 PostgreSQL·Redis 서비스 컨테이너를 띄워 실제 DB로 테스트하고, CD는 배포 시에만 GitHub Actions 러너 IP로 SSH 포트를 열었다가 끝나면 닫는 방식으로 보안과 자동화를 동시에 해결했습니다. 상시 SSH 포트를 여는 게 불안해서 만든 구조입니다.",
      },
    ],
    problemSolving: [
      {
        ...
        solution:
          "RDS를 제거하고 EC2 위 Docker PostgreSQL로 전환한 뒤, Spring Batch로 매일 새벽 3시에 pg_dump → S3 업로드 → 7일 이상 로컬 파일 정리를 자동화했습니다. S3 버킷에는 30일 수명주기 정책을 걸어 백업 스토리지 비용도 자동으로 통제됩니다.",
        result:
          "DB 관련 월 비용이 ~$15에서 S3 스토리지 비용 ~$0.1 수준으로 줄었고, RPO 24시간이라는 트레이드오프를 서비스 특성에 맞게 수용한 의사결정이었습니다.",
      },
      {
        issue:
          "새벽에 에러 알림 메일이 오면, 스택트레이스를 직접 읽고 심각도를 판단하는 과정이 반복돼서 피로감이 컸습니다.",
        analysis:
          "에러 로그를 그대로 전달하는 건 '알림'이지 '분석'이 아니었습니다. 혼자 운영하는 프로젝트에서 모든 에러를 직접 분류하는 건 지속 가능하지 않았고, AI로 1차 분석을 자동화하되 비용이 통제 가능해야 했습니다.",
        solution:
          "Logback 커스텀 Appender로 com.crossview 패키지의 ERROR 로그를 잡되, 알림 서비스 자체의 로그는 제외해 무한루프를 방지했습니다. 잡힌 에러는 @Async로 비동기 처리하면서 해시 기반 10분 쿨다운으로 중복을 걸러내고, Bedrock Haiku에 최근 50줄 컨텍스트와 함께 보내 원인·심각도·조치 방안을 받아 이메일·Slack으로 발송합니다. 스레드풀 상한 3개, 컨텍스트 50줄 제한으로 AI 호출 비용을 예측 가능한 범위 안에 묶었습니다.",
        result:
          "에러 알림에 AI 분석 결과가 함께 오니 '지금 봐야 하는 건지 아닌지'를 메일 제목만 보고 판단할 수 있게 됐고, Haiku 비용은 월 $1 미만으로 유지되고 있습니다.",
      },
      {
        issue:
          "배포 자동화를 위해 EC2 SSH 포트(22)를 상시 열어두는 것이 보안상 불안했습니다.",
        analysis:
          "CD 파이프라인이 SSH로 EC2에 접속해 배포하는데, 22번 포트를 0.0.0.0/0으로 열어두면 무차별 접속 시도에 노출됩니다. 배포는 하루 몇 번뿐인데 포트가 24시간 열려 있는 건 낭비이자 리스크였습니다.",
        solution:
          "GitHub Actions CD 워크플로우가 실행될 때 러너의 공인 IP를 조회해 해당 IP에 대해서만 SSH 포트를 열고, 배포가 끝나면(성공·실패 모두) Security Group 규칙을 삭제합니다. 배포 중에만 필요한 IP에만 포트가 열리는 구조입니다.",
        result:
          "SSH 포트 노출 시간이 24시간에서 배포 시 수 분으로 줄었고, 배포 자동화를 포기하지 않으면서 보안을 확보했습니다.",
      },
    ],
    retrospective: {
      improvements:
        "이 프로젝트에서 가장 많이 배운 건 '기능 구현'이 아니라 '운영 비용과 안정성 사이의 균형을 잡는 방법'이었습니다. RDS를 빼는 판단, AI 비용을 통제하는 설계, 배포 보안을 자동화하는 구조 — 전부 '이 서비스의 규모에 맞는 적정 수준이 어디인가'를 고민한 결과입니다.",
      regrets:
        "모니터링이 에러 알림에 치중되어 있고, 성능 메트릭(응답 시간, DB 쿼리 속도 등)에 대한 관측은 아직 부족합니다. CloudWatch 알람이 CPU 위주라 애플리케이션 레벨 모니터링을 보강해야 합니다.",
      futureWork:
        "Prometheus + Grafana로 애플리케이션 메트릭을 수집하고, 백업 복구 테스트를 자동화해서 RPO 24시간이 실제로 지켜지는지 정기적으로 검증하는 구조를 만들 계획입니다.",
    },
    links: {
