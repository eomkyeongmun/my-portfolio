...
export const projects: Project[] = [
  {
    ...
    techStack: [
      ...
      },
      {
        ...
        role: "단위 · 슬라이스 · 통합 테스트 계층 분리",
        reason:
          "모든 걸 @SpringBootTest로 짜면 느려서 안 돌리게 되고, 전부 목(mock)으로 짜면 쿼리가 실제로 실행되는지조차 검증하지 못합니다. 그래서 검증하려는 대상에 따라 도구를 나눴습니다. 비즈니스 규칙은 Mockito 단위 테스트로 빠르게, 요청 검증과 인증 파라미터 해석은 @WebMvcTest 슬라이스로, QueryDSL 동적 쿼리와 마이그레이션은 실제 PostgreSQL에 붙는 통합 테스트로 검증합니다. DB가 필요한 테스트는 환경변수 유무로 켜고 꺼서, DB 없는 로컬에서도 나머지 테스트는 항상 돌아가게 했습니다.",
      },
      {
        name: "AWS SDK 위 자체 AI 추상화 계층",
        role: "AI 호출 포트 분리 + 프롬프트 객체화",
        reason:
          "AI 기능을 늘리다 보니 에러 로그 분석과 자소서 분석이 각각 다른 방식으로 Bedrock을 호출하고 있었고, 프롬프트는 서비스 코드 안에 StringBuilder로 조립돼 있었습니다. AiChatClient라는 인터페이스 하나로 호출을 모으고 벤더 SDK를 구현체 안에 가둔 뒤, 프롬프트는 각각 독립된 타입(PromptTemplate 구현체)으로 분리했습니다. 프롬프트는 자주 바뀌는 '정책'이고 서비스는 '흐름'이라 변경 이유가 다르기 때문입니다. 덕분에 모델 교체는 설정 한 줄, 프롬프트 수정은 해당 클래스 하나만 건드리면 됩니다.",
      },
    ],
    problemSolving: [
      {
      ...
      },
      {
        ...
        solution:
          "그룹 행에 쓰기 잠금(SELECT ... FOR UPDATE)을 걸어 같은 그룹의 참여 처리를 직렬화했습니다. 낙관적 락도 검토했지만 멤버십 INSERT는 그룹 행을 수정하지 않아 버전이 올라가지 않고, 잠금 범위가 그룹 단위라 서로 다른 그룹의 참여는 서로를 기다리지 않으므로 비관적 락이 적절하다고 판단했습니다. 정원 판단은 컬렉션 크기 대신 COUNT 쿼리 결과로 바꿨습니다. 중복 가입은 여전히 unique 제약을 최종 방어선으로 두되, saveAndFlush로 제약 위반을 서비스 안에서 즉시 잡아 도메인 예외로 번역했습니다. save만 쓰면 INSERT가 커밋 시점까지 미뤄져 예외가 서비스 밖에서 터지고, 그러면 사용자에게 500이 나갑니다. 모집 신청 승인 경로도 같은 경쟁 상태여서 동일하게 적용했습니다.",
        result:
          "동일한 재현 테스트에서 10명 중 1명만 성공하고 나머지는 정원 초과로 거절됐으며, 최종 인원은 정확히 2명이었습니다. 이 테스트가 실제로 회귀를 잡는지 확인하려고 잠금을 다시 제거해봤더니 곧바로 실패(기대 1, 실제 10)해서, 테스트가 형식적으로 통과하는 게 아니라는 것도 검증했습니다. 경쟁 상태는 모킹으로는 재현 자체가 불가능하고 롤백되는 트랜잭션 테스트로도 여러 트랜잭션을 동시에 띄울 수 없어서, 실제 DB와 실제 스레드를 쓰는 통합 테스트로 분리했습니다.",
      },
      {
        issue:
          "그룹 멤버 목록을 여는 화면 하나가 멤버 수에 비례해 쿼리를 쏟아내고 있었습니다.",
        analysis:
          "멤버 목록은 각 멤버의 프로필과 그 그룹에 공유된 자소서를 함께 보여주는 화면인데, 구현이 멤버를 순회하면서 멤버마다 프로필 조회 1회, 공유 자소서 조회 1회를 날리는 구조였습니다. 6명이면 13번, 인원이 늘면 그만큼 선형으로 늘어납니다. 같은 문제를 코드 전반에서 찾아보니 세 종류가 있었습니다. (1) 순회하며 조회하는 전형적인 N+1, (2) 응답 DTO가 화면에 숫자 하나(현재 인원)를 표시하려고 group.getMemberships()를 건드려 멤버십 행 전체를 로딩하는 경우, (3) 연관 엔티티를 참조하는 목록 응답인데 fetch join도 EntityGraph도 없어 목록 크기만큼 추가 쿼리가 나가는 경우. 여기에 더해, PostgreSQL은 외래 키 제약을 걸어도 인덱스를 자동으로 만들어주지 않는다는 점을 놓치고 있어서 group_id·user_id 기준 조회가 전부 순차 스캔이었습니다.",
        solution:
          "(1)은 멤버 ID 목록을 모아 IN 절로 두 번의 조회로 합치고 메모리에서 매핑하도록 바꿔 쿼리 수를 멤버 수와 무관한 3회로 고정했습니다. (2)는 컬렉션을 로딩하는 대신 COUNT 집계로 바꾸고, 목록 조회에서는 그룹 ID들을 모아 GROUP BY로 한 번에 세도록 했습니다. (3)은 응답에 실제로 필요한 연관만 @EntityGraph로 지정했습니다. 인덱스는 Flyway 마이그레이션으로 조회 패턴에 맞춰 17개를 추가했습니다 — 단순히 FK마다 거는 게 아니라, '그룹의 질문 리스트를 대상자별로 최신순 조회' 같은 실제 쿼리 형태에 맞춰 복합 인덱스와 정렬 방향까지 맞췄습니다.",
        result:
          "멤버 6명 기준 13회였던 멤버 목록 쿼리가 3회로 줄었고, 인원이 늘어도 3회로 고정됩니다. N+1 회귀를 막기 위해 '멤버가 여러 명이어도 프로필·공유자소서 조회는 각각 1회'를 검증하는 테스트를 추가해서, 나중에 누가 다시 반복문 안에서 조회하면 테스트가 깨지도록 했습니다. 성능 수치보다도 '쿼리 개수가 데이터 양에 비례하지 않는다'는 성질을 테스트로 고정한 게 더 오래 가는 자산이라고 생각합니다.",
      },
      {
        issue:
          "일부 API에서 데이터 수정이 저장되지 않거나, 메일이 나간 뒤 정작 데이터는 롤백되는 문제가 있을 수 있는 구조였습니다.",
        analysis:
          "컨트롤러 네 곳이 서비스를 거치지 않고 Repository를 직접 주입받아 쓰고 있었습니다. 그러면 트랜잭션 경계가 없어서 조회·수정·저장이 각각 별개 트랜잭션으로 실행되고, JPA의 더티 체킹이 동작하지 않아 save()를 명시적으로 부르는 데 의존하게 됩니다. 반대 방향의 문제도 있었습니다. 그룹 초대·일정 알림·모집 신청 알림이 전부 @Transactional 안에서 SMTP를 직접 호출하고 있었는데, 이건 DB 커넥션을 붙잡은 채 외부 서버 응답을 기다리는 구조라 커넥션 풀 고갈로 이어질 수 있고, 더 나쁜 건 뒤에서 예외가 나 롤백돼도 메일은 이미 나가버려 되돌릴 수 없다는 점이었습니다.",
        solution:
          "컨트롤러에 있던 로직을 서비스로 올려 트랜잭션 경계를 명확히 하고, 컨트롤러는 요청 검증과 응답 변환만 담당하게 했습니다. 메일 발송은 직접 호출을 걷어내고 도메인 이벤트를 발행한 뒤 @TransactionalEventListener(AFTER_COMMIT)에서 처리하도록 바꿨습니다. 커밋이 확정된 뒤에만 메일이 나가므로 롤백된 작업의 알림이 나가지 않고, SMTP 호출이 트랜잭션 밖으로 빠집니다. 인증 코드 발송처럼 트랜잭션 없이 발행되는 이벤트도 있어서 fallbackExecution 옵션으로 그 경우엔 즉시 실행되게 했습니다(기본값이면 조용히 버려집니다). 부수적으로, 알림 수단이 늘어나도 비즈니스 서비스는 건드리지 않게 됐습니다.",
        result:
          "수정 API가 더티 체킹으로 동작하게 되면서 불필요한 save() 호출이 사라졌고, 이를 테스트로 고정했습니다(수정 시 save가 호출되지 않는지 검증). 트랜잭션 안에서 외부 I/O를 호출하는 지점은 0이 됐습니다. 이 작업을 하면서 '계층을 나누는 건 코드를 예쁘게 두려는 게 아니라 트랜잭션·예외·재사용의 경계를 만드는 일'이라는 걸 체감했습니다.",
      },
      {
        issue:
    ...
    retrospective: {
      improvements:
        "전반부에서 가장 많이 배운 건 '운영 비용과 안정성 사이의 균형'이었습니다. RDS를 빼는 판단, AI 비용을 통제하는 설계, 배포 보안을 자동화하는 구조 — 전부 '이 서비스의 규모에 맞는 적정 수준이 어디인가'를 고민한 결과입니다. 후반부에는 관심사가 안쪽으로 옮겨갔습니다. 기능이 다 도는 상태에서 코드를 다시 읽으며 '이게 사용자가 몰리면 버티는 구조인가'를 따져봤고, 그때 나온 것이 정원 초과 경쟁 상태, 멤버 수에 비례해 늘어나는 쿼리, 트랜잭션 경계 없이 실행되던 API, 이력이 남지 않는 스키마 관리였습니다. 전부 '지금 당장은 잘 도는데 조건이 바뀌면 무너지는' 종류였고, 이런 건 기능 테스트로는 절대 드러나지 않는다는 걸 배웠습니다. 그래서 문제마다 재현 테스트를 먼저 쓰고 고치는 순서를 지켰고, 특히 동시성 수정에서는 고친 뒤 일부러 다시 되돌려 테스트가 실패하는지 확인했습니다. 테스트가 통과하는 것보다 '틀렸을 때 실패하는지'가 더 중요하다는 걸 그때 실감했습니다.",
      regrets:
